...
const verbs = ["actualize", "administrate", "aggregate", "architect", "benchmark", "brand", "build", "communicate", "conceptualize",
...
const adjectives = ["24/7", "24/365", "accurate", "adaptive", "alternative", "an expanded array of", "B2B", "B2C", "backend",
...
const nouns = ["action items", "alignments", "applications", "architectures", "bandwidth", "benefits",
...
export function generateBullshit(): string {
  return `${adverbs[getRandomInt(0, adverbs.length - 1)]} ${verbs[getRandomInt(0, verbs.length - 1)]} ${adjectives[getRandomInt(0, adjectives.length - 1)]} ${nouns[getRandomInt(0, nouns.length - 1)]}`;
}
